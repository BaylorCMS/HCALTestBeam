...
private:
    ...
    std::vector<double> binsQIE8, binsQIE11, binsMIP, binsMIP8, binsChris;
...
void runHBHEcollection(C& DigiCollection, std::map<std::string, float>& times, edm::Handle<HcalTBTriggerData>& trigData,std::map<std::string, float>& towerSum, std::map<std::string, float>& towerSumCal, const std::vector<int>& pedestal, const std::vector<int>& reco){
	...
	{    
	    ...
	    fillHist(hists, "beam_adc_binChris_" + hnum.str(), q, 333, binsChris.data());
	    fillHist(hists, "beam_adc_constBin_" + hnum.str(), q, 219355, 0, 680000);
	...
	fillHist(hists, "adc_nosub_binChris_" + hnum.str(), qNosub, 333, binsChris.data());
	fillHist(hists, "adc_nosub_constBin_" + hnum.str(), qNosub, 219355, 0, 680000);
    }
}
...
void adcHists::beginJob()
{
    ...
    for(double& binEdge : edgesChris)
    {
        binsChris.push_back(binEdge/gain_);
    }
    //for(unsigned int i = 0; i < binsChris.size()-1; i++)
    //{
    //  std::cout<<i+1<<" - "<<i<<"  "<<binsChris[i+1]-binsChris[i]<<std::endl;
    //}
